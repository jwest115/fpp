...
==== Sizeof Expressions
A *sizeof expression* computes the size of a type. It 
evaluates to an integer value representing the maximum number of bytes
required to represent any value of the specified type. For example:
[source,fpp]
----
constant i = sizeof(U32)
----

In this example, constant `i` has a value 4.
...
<<Defining-Types_Array-Type-Definitions_Type-Names,in a later section of this 
manual>>.)
...
The following model is valid because it defines and uses `FW_FIXED_LENGTH_STRING_SIZE`:

[source,fpp]
----
constant FW_FIXED_LENGTH_STRING_SIZE = 80

constant a = FW_FIXED_LENGTH_STRING_SIZE
----

The following model is invalid, because it uses `FW_FIXED_LENGTH_STRING_SIZE`
without defining it:
...
[source,fpp]
--------
constant FW_FIXED_LENGTH_STRING_SIZE = "abc"
--------
...
constant FW_FIXED_LENGTH_STRING_SIZE = "abc"
^
error: the F Prime framework constant FW_FIXED_LENGTH_STRING_SIZE must have an integer type
----
