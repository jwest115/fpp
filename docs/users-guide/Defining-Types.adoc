...
The `string` type name requires that the `FwSizeStoreType`
<<Defining-Types_Framework-Types,framework type>> 
and `FW_FIXED_LENGTH_STRING_SIZE` 
<<Defining-Constants_Framework-Constants,framework constant>> is defined
in the model.
The framework constant `FW_FIXED_LENGTH_STRING_SIZE` specifies the 
maximum string length of any value of type `string` when it
is represented in the F Prime framework.
For example:
+
[source,fpp]
----
# Framework type required when using strings
type FwSizeStoreType = U16
# Framework constant required when using strings without a size
constant FW_FIXED_LENGTH_STRING_SIZE = 256
# A is an array of 3 strings with size FW_FIXED_LENGTH_STRING_SIZE
array A = [3] string
----

* The type name `string size` _e_, where _e_ is a numeric expression
specifying the maximum string length of any value of that type.
The `string size` type name requires that the `FwSizeStoreType` framework
type is defined in the model. Note that it does _not_ require the
`FW_FIXED_LENGTH_STRING_SIZE` framework constant to be defined since the size
is specified by the _e_ numeric expression.
+
[source,fpp]
----
# Framework type required when using strings
type FwSizeStoreType = U16
# A is an array of 3 strings with size 40
