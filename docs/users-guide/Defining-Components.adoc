...
[source,fpp]
----
# The following definitions are required when using string types
...
<<Defining-Modules,module>> or 
<<Defining-State-Machines_Constants-Types-and-Enums,
...
In general, it is a good idea to put a definition inside a component
...
In this case it can be useful to break up the
...
Include specifiers can also appear at the top level of a
model, inside a module definition, inside a state machine
definition, or inside a
topology definition.
