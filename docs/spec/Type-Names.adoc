...
not present, then the translator uses the default maximum string specified by
FW_FIXED_LENGTH_STRING_SIZE.

==== Implied Uses
String type names require that the `FwSizeStoreType`
<<Definitions_Framework-Definitions_Type-Definitions,framework type>>
is defined in the model. If a string uses the default size (i.e., does not 
include a `size` expression), the `FW_FIXED_LENGTH_STRING_SIZE` 
